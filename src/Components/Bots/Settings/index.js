  ...
  renderSettingsForm() {
    ...
    for (const [settingKey, setting] of _.entries(this.state.settings)) {
      switch (settingKey) {
        ...
        default:
          if (this.props.bot.info.connectionType === 'player') {
    ...
    return <form onSubmit={this.updateBotSettings}>{settings}</form>;
  }
  ...
  renderSettingInput(settingKey, setting) {
    ...
    return (
      <div key={settingKey} className="row">
        <div key={`${settingKey}label`} className="settings-label col-md-4">
          {settingKey}
        </div>
        <div className="col-md-4">
          <input
            onChange={this.handleInputChange}
            key={`${settingKey}input`}
            type="textarea"
            name={settingKey}
            value={settingValue}
          />
  ...
  render() {
    return (
      <div id="settings">
        ...
        <Button bsStyle="danger" onClick={this.deleteBot}>
          Delete Bot
        </Button>
      </div>
    );
